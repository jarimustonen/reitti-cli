...
The maintainer authorized public visibility and the first v1.0.0 publication. On 2026-09-14 the maintainer granted standing authorization for an agent to decide and execute future releases after an explicit stint and terminal handoff when the reservation-aware issue DAG is completely empty. This autonomy is still gated by a clean pushed `main`, every repository and release-time check, SemVer and changelog preparation, bounded live smoke tests, and verification of the actual publication jobs and assets. A non-empty or unverifiable DAG, failed gate, unresolved decision, or ambiguous publication state blocks release. Ordinary push and pull-request CI is intentionally absent by maintainer decision and must not be regenerated merely to satisfy an audit. The generated tag-triggered cargo-dist workflow remains the release path; local checks are not a substitute for a successful release run.
